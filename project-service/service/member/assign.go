...
func failedMessage(request *resource.AssignRequest, resource resource.AssignResource) string {
	return fmt.Sprintf("Failed %s members %s to project [%s]", getPrefixMessage(request.IsAssign()), resource.Members, request.TeamId)
}

func successMessage(request *resource.AssignRequest, resource resource.AssignResource) string {
	return fmt.Sprintf("Successfully %s members %s to project [%s]", getPrefixMessage(request.IsAssign()), resource.Members, request.TeamId)
}
