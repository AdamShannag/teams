...
// QueryAssignBy chains the current query on the "assignBy" edge.
func (tq *TeamQuery) QueryAssignBy() *MemberQuery {
...
func (tq *TeamQuery) QueryAssignation() *ProjectQuery {
...
func (tq *TeamQuery) Clone() *TeamQuery {
	...
	return &TeamQuery{
		...
		withAssignBy:    tq.withAssignBy.Clone(),
		withAssignation: tq.withAssignation.Clone(),
...
// WithAssignBy tells the query-builder to eager-load the nodes that are connected to
// the "assignBy" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TeamQuery) WithAssignBy(opts ...func(*MemberQuery)) *TeamQuery {
	query := (&MemberClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withAssignBy = query
	return tq
}
...
func (tq *TeamQuery) WithAssignation(opts ...func(*ProjectQuery)) *TeamQuery {
	query := (&ProjectClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withAssignation = query
	return tq
}
...
func (tq *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := tq.withAssignBy; query != nil {
		if err := tq.loadAssignBy(ctx, query, nodes, nil,
			func(n *Team, e *Member) { n.Edges.AssignBy = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withAssignation; query != nil {
		if err := tq.loadAssignation(ctx, query, nodes, nil,
			func(n *Team, e *Project) { n.Edges.Assignation = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TeamQuery) loadAssignBy(ctx context.Context, query *MemberQuery, nodes []*Team, init func(*Team), assign func(*Team, *Member)) error {
...
func (tq *TeamQuery) loadAssignation(ctx context.Context, query *ProjectQuery, nodes []*Team, init func(*Team), assign func(*Team, *Project)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Team)
	for i := range nodes {
...
func (tq *TeamQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withAssignBy != nil {
			_spec.Node.AddColumnOnce(team.FieldAssignedBy)
		}
		if tq.withAssignation != nil {
			_spec.Node.AddColumnOnce(team.FieldProjectID)
		}
	}
